...
export default function RecruiterProfile({ user, onProfileUpdated }: RecruiterProfileProps) {
  ...
  return (
    <main className="p-4 md:p-8 min-h-screen bg-[#f8f9fa] text-slate-800 font-sans">
      <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 md:gap-6 px-3 sm:px-4 md:px-6 lg:px-28 max-w-7xl mx-auto">
        ...
        <div className="w-full lg:w-[320px] shrink-0 space-y-4 md:space-y-6">
          ...
          <div className="bg-white rounded-lg md:rounded-[24px] p-4 md:p-6 shadow-sm border border-slate-100">
            ...
            <div className="space-y-3 md:space-y-5">
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 md:mb-2 block">Title</span>
                <p className="text-xs sm:text-sm font-medium text-slate-900">
                  {recruiterProfile?.title || "—"}
                </p>
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 md:mb-2 block">Tagline</span>
                <p className="text-xs sm:text-sm font-medium text-slate-900">
                  {recruiterProfile?.tagline || "—"}
                </p>
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 md:mb-2 block">Experience</span>
                <p className="text-xs sm:text-sm font-medium text-slate-900">
              ...
              <div>
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 md:mb-2 block">Rating</span>
                <p className="text-xs sm:text-sm font-medium text-slate-900">
        ...
        <div className="flex-1 space-y-4 md:space-y-6">
          ...
          <div className="flex flex-col xl:flex-row gap-4 md:gap-6">
            {/* User Main Card */}
            <div className="bg-white rounded-lg md:rounded-[24px] p-4 md:p-6 shadow-sm border border-slate-100 flex-1 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4 md:gap-6 relative">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 md:gap-6 flex-1 min-w-0">
                ...
                <div className="text-center sm:text-left space-y-1 md:space-y-2 flex-1 min-w-0">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 text-xs sm:text-sm text-slate-600 justify-center sm:justify-start flex-wrap">
          ...
          <div className="bg-white rounded-lg md:rounded-[24px] p-4 md:p-6 shadow-sm border border-slate-100 space-y-3 md:space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-base md:text-lg text-slate-900">About</h2>
            </div>
          ...
          <div className="bg-white rounded-lg md:rounded-[24px] p-4 md:p-6 shadow-sm border border-slate-100 space-y-3 md:space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 md:gap-3">
              <h2 className="font-bold text-base md:text-lg text-slate-900">Skills & Expertise</h2>
              <Button
                ...
                {Object.entries(
                  ...
                  <div key={type}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {tags.map((tag) => (
                        <span key={tag.id} className="bg-blue-100 text-blue-700 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm">
                          {tag.skill.value}
                        </span>
                      ))}
                    </div>
          ...
          <div className="bg-white rounded-lg md:rounded-[24px] p-4 md:p-6 shadow-sm border border-slate-100 space-y-3 md:space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 md:gap-3">
              <h2 className="font-bold text-base md:text-lg text-slate-900">Links & Contacts</h2>
              <Button
              ...
              <div className="space-y-2">
                {recruiterProfile.links.map(link => (
                  <a 
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block text-xs sm:text-sm text-blue-600 hover:underline break-all"
                  >
                    {link.label || link.url}
                  </a>
                ))}
          ...
          <div className="bg-white rounded-lg md:rounded-[24px] p-4 md:p-6 shadow-sm border border-slate-100 space-y-3 md:space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 md:gap-3">
              <h2 className="font-bold text-base md:text-lg text-slate-900">Active Job Searches</h2>
              <Button
              ...
              <div className="space-y-2">
                {recruiterProfile.activeSearches.map(search => (
                  <div key={search.id} className="border border-slate-200 rounded-lg p-3">
                    <p className="font-medium text-xs sm:text-sm text-slate-900">{search.title}</p>
          ...
          {recruiterProfile.insights && recruiterProfile.insights.filter(i => i.status === "PUBLISHED").length > 0 && (
            <div className="bg-white rounded-lg md:rounded-[24px] p-4 md:p-6 shadow-sm border border-slate-100 space-y-3 md:space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 md:gap-3">
                <h2 className="font-bold text-base md:text-lg text-slate-900">Insights & Content</h2>
                <Button
              ...
              <div className="flex flex-wrap justify-center gap-3 md:gap-6">
                {recruiterProfile.insights
                  ...
                  .map(insight => (
                    <div
                      key={insight.id}
                      className="w-full sm:w-80 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"
                    >
      ...
      <Dialog open={isEditProfileOpen} onOpenChange={setIsEditProfileOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto p-4 sm:p-6">
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg">Edit Recruiter Profile</DialogTitle>
          </DialogHeader>
          <RecruiterManager 
      ...
      <Dialog open={isLinksDialogOpen} onOpenChange={setIsLinksDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto p-4 sm:p-6">
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg">Edit Links & Contacts</DialogTitle>
          </DialogHeader>
          <LinksManager 
      ...
      <Dialog open={isSearchesDialogOpen} onOpenChange={setIsSearchesDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto p-4 sm:p-6">
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg">Edit Active Job Searches</DialogTitle>
          </DialogHeader>
          <ActiveSearches 
      ...
      <Dialog open={isInsightsDialogOpen} onOpenChange={setIsInsightsDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto p-4 sm:p-6">
          <DialogHeader>
            <DialogTitle className="text-base sm:text-lg">Edit Insights & Content</DialogTitle>
          </DialogHeader>
          <InsightsManager 
